...
class LeonovaARadixMergeSortALL : public BaseTask {
 public:
  static constexpr ppc::task::TypeOfTask GetStaticTypeOfTask() {
    return ppc::task::TypeOfTask::kALL;
  }

  explicit LeonovaARadixMergeSortALL(const InType &in);
 ...
 private:
  ...
  static void ParallelRadixSort(std::vector<int64_t> &arr, size_t left, size_t right);

  static void SimpleMerge(std::vector<int64_t> &arr, size_t left, size_t mid, size_t right);

  static void MergeChunks(std::vector<int64_t> &arr, const std::vector<size_t> &boundaries);

  static void MergeSortedVectors(std::vector<int64_t> &local, const std::vector<int64_t> &incoming);

  static void HierarchicalMerge(std::vector<int64_t> &local_data, int rank, int world_size);

  static void BroadcastResult(std::vector<int64_t> &local_data, int rank, int total);
